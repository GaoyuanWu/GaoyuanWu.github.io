...
# Some Previous Work
**Gaoyuan Wu**, Sam Smith, Maria Garlock, *Shape optimization of box-type structures under waves using multi-objective Bayesian Optimization: Reducing wave loads and improving wave attenuation*, **Engineering Structures**, August 2026, Volume 360, DOI: [10.1016/j.engstruct.2026.122780](https://doi.org/10.1016/j.engstruct.2026.122780)

<img src="/images/MOBO_SPH_framework.png" alt="drawing" width="800"/>
* Box-type coastal structures—including breakwaters and box-girder bridges—face growing threats from intensifying coastal hazards, where structural shape critically influences wave–structure interaction. Existing shape optimization frameworks are limited by simplified wave theory and single-objective formulations. This work proposes a novel methodology coupling Smoothed Particle Hydrodynamics (SPH) with Multi-Objective Bayesian Optimization (MOBO) to optimize the shape of box-type coastal infrastructure under realistic wave loads. In a single-objective problem, the optimal design improves wave attenuation by 60% over a conventional rectangular breakwater. In the multi-objective setting, a Pareto-optimal set of designs is identified that simultaneously reduces wave forces and improves wave attenuation—all outperforming the traditional rectangular design. A parallelization scheme further reduces computational time by 73%.
